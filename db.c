...
//Mutex used for the First phase, where we use pthread_lock()
//pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;

//Initilize the semaphore, a variable to store its value, and a flag to be used later
...
{
    //Initialize the semaphore with 0, since we dont want it to share fork() processes
    //and 1 so that either write or read operation is running 
...
int db_add(DB* self, Variant* key, Variant* value)
{
    ...
    //Reduce the value of the semaphore by 1, so that it becomes 0
	...
	//Wait until the merge thread is finished and after it's done, unlock the cv_lock,
	//since try_lock locks the lock
    ...
    //Store the return of memtable_add, so that memtable_add is executed before db_add is finished
    int result = memtable_add(self->memtable, key, value);

    //First phase code
    //pthread_mutex_unlock(&mutex);

    //Incrrement the value of the semaphore and set it back to 1
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    //Update the mutual_value with the value of the semaphore
    //so that while can check if it is equal to 0.
    //If it is 0, that means that the write thread is running,
    //so any read thread should wait
    sem_getvalue(&mutual_exclusion,&mutual_value);
    while(mutual_value == 0){
      sem_getvalue(&mutual_exclusion,&mutual_value);
    }

    //While a read thread is running, the write thread shouldn't run,
    //so we call sem_wait and set the semaphore value to 0.
    //That way, when the write thread calls db_add and it executes sem_wait
    //the value becomes -1 and the add operation is added in the queue of the semaphore,
    //waiting for the get operations to be done
    if(i == 0){
      i = 1;
      sem_wait(&mutual_exclusion);
      sem_getvalue(&mutual_exclusion,&mutual_value); 
    ...
    if (memtable_get(self->memtable->list, key, value) == 1){
      ...
      //After the read threads are done, they increment the value of the semaphore,
      //so that the write thread that is waiting in the queue, gets released
      if(i ==1){
	i = 0;
         sem_post(&mutual_exclusion);
	 sem_getvalue(&mutual_exclusion,&mutual_value);
    ...
    //Store the return of the sst_get, so that sst_get gets executed before db_get is finished
